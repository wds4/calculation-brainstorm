        ...
        if (result_exists.rows[0].exists == true) {
          fetch(testUrl)
            ...
            .then((data) => async() => {
              ...
              const { url } = await put('blob.txt', 'Hello World!', { access: 'public' });
              console.log(url)
              res.status(200).json({ message: 'pubkey ' + pubkey1 + ' already exists in the customer database; ratingsTable requested from the interp engine; here is what we received: ' + JSON.stringify(data) })
            })
        } else {
